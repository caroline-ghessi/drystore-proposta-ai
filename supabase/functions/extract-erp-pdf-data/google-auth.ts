// Google OAuth and JWT authentication utilities for ERP PDF processing
...
export class GoogleAuthManager {
  ...
  async getCachedGoogleAccessToken(): Promise<string> {
    console.log('🔑 Getting cached Google OAuth2 access token for ERP processing...');
    ...
    console.log('✅ New access token cached successfully for ERP processing');
    return this.accessTokenCache;
  }
  ...
  private async getGoogleAccessTokenWithRetry(): Promise<GoogleTokenResponse> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`🔑 Getting Google OAuth2 access token for ERP (attempt ${attempt}/${maxRetries})...`);
        ...
        console.log('✅ Google OAuth2 token obtained successfully for ERP:', {
  ...
  private async signJWTFallback(data: string, privateKey: string): Promise<string> {
    console.log('⚠️ Using fallback JWT signing method for ERP');
